...
class Vertex:
  ...
  def Collinear(a,b,c):
    ...
    return (( c.v.z - a.v.z ) * ( b.v.y - a.v.y ) -( b.v.z - a.v.z ) * ( c.v.y - a.v.y ) == 0 
            and ( b.v.z - a.v.z ) * ( c.v.x - a.v.x ) -( b.v.x - a.v.x ) * ( c.v.z - a.v.z ) == 0
            and ( b.v.x - a.v.x ) * ( c.v.y - a.v.y ) - ( b.v.y - a.v.y ) * ( c.v.x - a.v.x ) == 0 )
...
vertex %s
endloop
endfacet"""%(self.vertex[2],self.vertex[1],self.vertex[0])
  def xyz(self):
    return """%s,%s,%s"""%(self.vertex[2],self.vertex[1],self.vertex[0])
  def debug(self):
...
class Hull:
  ...
  def DoubleTriangle(self):
    '''Builds initital double triangle.Finds the 3 non collinear points and makes 2 faces out of them 
    in opposite order. It then finds a fourth point that is not coplanar with that face.  The  
    ...
    while(Vertex.Collinear(self.vertices[v0%nv],self.vertices[(v0+1)%nv],self.vertices[(v0+2)%nv])):
      v0 = (v0+1)%nv 
      if v0==0:
        raise Exception("Double triangle: all points are collinear!")
    v1=(v0+1)%nv
    v2=(v1+1)%nv
  ...
  def CleanUp(self,ev,v):
    """
		...
		flags and NULLs out some pointers.  
		"""
    de=self.CleanEdges()
  ...
  def CleanEdges(self):
    """
		...
		visible face and NULL out newface. It also deletes marked edges.
		"""
  ...
  def CleanVertices(self,evi,vi):
    ...
    for v in self.vertices:
      ...
      v.onhull = not ONHULL
    return evi,vi
"""inputxyz is used for reading the .xyz file and giving as input"""
def inputxyz(filepath):
    ...
    with open(filepath, "r") as f:
        ...
        # Read the point coordinates and store them as NumPy arrays
        points = []
        newlist=[]
        ...
        #print(newlist)
        coordinates = np.array(newlist)
       # print(coordinates)
       # print(coordinates.shape)
        coordinates = coordinates.reshape(num_points,3)
       # print(coordinates.shape)
...
if __name__ == "__main__":
 ...
 # h.Print()
  h.PrinttoFile("E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\out.stl")
  #h.PrinttoFile("/content/gdrive/My Drive/Colab Notebooks/out.stl")

  h1=Hull(cubef)
  poly_list=[]
  poly_list=inputxyz("E:/ed22d003_cg_assmt3/ed22d003_cg_assmt3/teapot_306.xyz")
  h2=Hull(poly_list)
  #h2.Print()
  h2.PrinttoFile("E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\outpoly.stl")
  poly_list1=inputxyz("E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\_uniform.xyz")
  h2=Hull(poly_list1)
  #h2.Print()
  h2.PrinttoFile("E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\out1.stl")



import subprocess

# Set the path to your MeshLab executable
meshlab_path = "C:/Program Files/VCG/MeshLab/meshlab.exe"
# Set the path to your STL file
stl_path = "E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\out.stl"

# Open MeshLab and load the STL file
subprocess.call([meshlab_path, stl_path])
# Set the path to your STL file
stl_path = "E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\outpoly.stl"

# Open MeshLab and load the STL file
subprocess.call([meshlab_path, stl_path])

stl_path = "E:\ed22d003_cg_assmt3\ed22d003_cg_assmt3\out1.stl"
